...
namespace cyy::compiler {
  ...
  DFA NFA_to_DFA(const NFA &nfa)  {
	...
	std::vector<std::set<state_type>> subsets{nfa.epsilon_closure({nfa.get_start_state()})};
  ...
  return {DFA_states,nfa.get_alphabet(),0,DFA_transition_table,DFA_final_states};
  }
