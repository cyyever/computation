...
DFA DFA::minimize() const {
  ...
  while(true) {
	...
	while(it!=group.end()) {
	  ...
	  for(auto & sub_group:sub_groups) {
		...
		if(in_group && state_location[ move(*(sub_group.begin()),a)]!= state_location[move(state,a)]) {
		in_group=false;
		}
		});
	    if(in_group) {
  ...
  std::map<std::pair<uint64_t, symbol_type>, uint64_t> minimize_DFA_transition_table;
  for(size_t i=0;i<groups.size();i++) {
  ...
  return {minimize_DFA_states, alphabet->name(), minimize_DFA_start_state,minimize_DFA_transition_table,
          minimize_DFA_final_states};
}
} // namespace cyy::lang
